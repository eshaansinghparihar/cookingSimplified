...
function SearchRecipeSuggestion({dishName}){
    const classes = useStyles();
    const [recipeList,setRecipeList]=useState([]);
    const [id,setId]=useState(0);
    const [open, setOpen] = useState(false);
    var apiDish="https://api.spoonacular.com/recipes/complexSearch?apiKey=9f4aa7cc7b3448c688bb9e36b0b124ca&number=40&query="+dishName;
    useEffect(()=>{
        fetch(apiDish)
            .then(response => response.json())
            .then(recipeData=> {
            setRecipeList(recipeData.results);
        })
        .catch(errorHandler)
    },[dishName]);
    ...
    function RecipeDetail(){
        ...
        var apiRecipe="https://api.spoonacular.com/recipes/"+id+"/information?apiKey=9f4aa7cc7b3448c688bb9e36b0b124ca&includeNutrition=true";   
        useEffect(()=>{
            fetch(apiRecipe)
                .then(response => response.json())
                .then(recipe => {
...
export default SearchRecipeSuggestion;